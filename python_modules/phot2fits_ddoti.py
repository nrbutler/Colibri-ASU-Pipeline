...
def phot2fits(timefile,masterfile,t0,cfilter='GAIA-EDR3 G(AB)'):
    ...
    if ('TCSTART' in hdr): tc0 = hdr['TCSTART']
    if ('TCSTOP' in hdr): tc1 = hdr['TCSTOP']
    ...
    ref_cat_list = glob('ps1_stack_*_dir/calibrated_catalog.fits')
    if (len(ref_cat_list)>0): ref_cat = ref_cat_list[0]
    else: ref_cat='nullfile.txt'
