...
from numpy import sqrt,log10,median,zeros,abs,where
...
def calibrate(infile,calfile,sys_err=0.01,cal_mag_min=14.0,cal_mag_max=19.0,do_plot=False,quick_cal=False,mag0=0.):
    ...
    if (nn>0):
        ...
        if (os.path.exists(gcal_file)):
            ...
            delta_mag = abs( mag0 - mag - median(mag0-mag) )
            cal_scatter = 1.48*median(delta_mag)
            wg = where(gphot)[0]
            good_cal = delta_mag<2*cal_scatter
            if (good_cal.sum()<10): good_cal = gphot[gphot]
            writeto(gcal_file,wg[good_cal])

        mag_offset = mag0[good_cal]-mag[good_cal]
        cal_scatter = 1.48*median(abs(mag_offset-median(mag_offset)))
        ...
        print (""" Calibration Scatter %.4f""" % (cal_scatter))
        ...
        gl = (mag_all<25)*(dmag_all0>0.01)*(dmag_all0<0.1)
        mag_all += mag_offset
        ...
        if (do_plot):
            ...
            ax.plot (mag0[good_cal],(mag0-mag_all[gphot])[good_cal],'go',alpha=0.3)
            ax.plot ([x0,x1],[0.,0.],':')
            ax.set_ylim((-0.5,0.5))
            #ax.set_ylim((-0.2,0.2))
