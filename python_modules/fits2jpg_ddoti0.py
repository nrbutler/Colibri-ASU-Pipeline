...
from numpy import sqrt,log,zeros,arcsinh,pi,arctan2,cos,array,ceil
...
def fits2jpg(file0,out_size=1024,wt_min=0,invert=True,logscale=False,zscale=True,log_fac=5.,make_thumbs=False,ts=301,te=15,ttag="",tag_label=""):
    """
    """
    ...
    if (make_thumbs):
        ...
        for i in range(0,nt_x):
            padx1=padx2=0
            if (i==0): padx1 = te-i0
            if (i==nt_x-1): padx2 = te+i0-extra_x
            for j in range(0,nt_y):
                pady1=pady2=0
                if (j==0): pady1 = te-j0
                if (j==nt_y-1): pady2 = te+j0-extra_y
...
if __name__ == "__main__":
    ...
    save_fits=False
    if (len(sys.argv)>7): 
        if (sys.argv[7]=="save_fits"): save_fits=True

    tag_label=""
    if (len(sys.argv)>8): tag_label=sys.argv[8]

    ttag=""
    if (len(sys.argv)>9): ttag=sys.argv[9]
    
    fits2jpg(file0,out_size=sz,invert=invert,logscale=logscale,zscale=zscale,make_thumbs=make_thumbs,ts=thumb_size,te=thumb_edge,ttag=ttag,tag_label=tag_label)
