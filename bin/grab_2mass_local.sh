...
cat_dir=${REDUX_BASE_DIR}/catalogs/2mass
...
for dec in $decs; do
    ...
    for ra in $ras; do
        echo grab_2mass_local.py ${cat_dir}/2mass_psc_${ra}_${dec}.fits.gz 2mass_psc_${ra}_${dec}_${cam}.fits $ra0 $dra1 $dec0 $ddec 
        grab_2mass_local.py ${cat_dir}/2mass_psc_${ra}_${dec}.fits.gz 2mass_psc_${ra}_${dec}_${cam}.fits $ra0 $dra1 $dec0 $ddec &
    done
done
wait
...
files=glob('2mass_psc_*_*[0-9]_${cam}.fits')
n=0
...
writeto('2mass_radec_${cam}.fits',data,overwrite=True)" | python3

n=`gethead NAXIS1 2mass_radec_${cam}.fits`
if [ "$n" -eq 0 ]; then
    rm 2mass_radec_${cam}.fits
else
    radec2radec.py $file 2mass_radec_${cam}.fits
fi

n=`gethead NAXIS1 2mass_radec_${cam}.fits`
[ "$n" -eq 0 ] && rm 2mass_radec_${cam}.fits

rm 2mass_psc_*_*_${cam}.fits 2>/dev/null
