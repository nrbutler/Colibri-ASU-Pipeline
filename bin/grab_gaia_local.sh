...
cat_dir=${REDUX_BASE_DIR}/catalogs/gaia

while [ $# -gt 0 ] ; do eval $1 ; shift ; done

mjd=`gethead MJD-OBS $file`
[ "$mjd" ] || mjd=57388.0
...
for dec in $decs; do
    ...
    for ra in $ras; do
        echo grab_gaia_local.py ${cat_dir}/gaia_edr3_${ra}_${dec}.fits.gz gaia_edr3_${ra}_${dec}_${cam}.fits $ra0 $dra1 $dec0 $ddec $mjd
        grab_gaia_local.py ${cat_dir}/gaia_edr3_${ra}_${dec}.fits.gz gaia_edr3_${ra}_${dec}_${cam}.fits $ra0 $dra1 $dec0 $ddec $mjd &
    done
done
wait
...
files=glob('gaia_edr3_*_*[0-9]_${cam}.fits')
n=0
for file in files: n += getheader(file)['NAXIS1']
data = zeros((5,n),dtype='float32')
n0=0
for file in files:
...
writeto('gaia_radec_${cam}.fits',data,overwrite=True)" | python3

n=`gethead NAXIS1 gaia_radec_${cam}.fits`
if [ "$n" -eq 0 ]; then
    rm gaia_radec_${cam}.fits
else
    radec2radec.py $file gaia_radec_${cam}.fits
fi

n=`gethead NAXIS1 gaia_radec_${cam}.fits`
[ "$n" -eq 0 ] && rm gaia_radec_${cam}.fits

rm gaia_edr3_*_*_${cam}.fits 2>/dev/null
