...
xyradius=2000.0 # pixels from center
inst=coatli
nstars=1000
thresh=20.0
...
base=${file%'.fits'}
if [ -d ${base}_dir ]; then
    echo "Not re-creating ${base}_dir"
else
    run_sex.sh $file $inst -DETECT_THRESH $thresh -CHECKIMAGE_TYPE BACKGROUND -CHECKIMAGE_NAME back.fits
    imarith $file ${base}_dir/back.fits sub tmp/$file
    mv tmp/$file $file
fi

#grep -v '#' ${base}_dir/${base}_radec.txt | sort -n -k 3 | awk '{if(NR<='$nstars') print $8,$9}' > xy$$.txt
#grep -v '#' ${base}_dir/${base}_radec.txt | sort -n -k 3 | awk 'BEGIN{n=0}{dx=$8-'$x0';dy=$9-'$y0';d=sqrt(dx*dx+dy*dy); if(d<'$xyradius' && $4>=5.e-4) {n=n+1; if(n<='$nstars') print $8,$9}}' > xy$$.txt
grep -v '#' ${base}_dir/${base}_radec.txt | sort -n -k 3 | awk 'BEGIN{n=0}{dx=$8-'$x0';dy=$9-'$y0';d=sqrt(dx*dx+dy*dy); if(d<'$xyradius') {n=n+1; if(n<='$nstars') print $8,$9}}' > xy$$.txt
