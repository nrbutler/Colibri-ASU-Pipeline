...
ps1_file=ps1_stack_${filter}.fits
if [ -f "$ps1_file" ]; then
    pfilter=$filter
else
    [ "$filter" = "B" ] && pfilter=g
    [ "$filter" = "gri" ] && pfilter=r
    [ "$filter" = "zy" ] && pfilter=z
...
if [ "$make_thumbs" = "yes" ]; then
    ...
    ls thumb_*_*_*_*_*_.jpg | sed -e 's/\.jpg//' | awk -F_ '{print "">"radec_"$2"_"$3"_.txt"}'
    ...
    function circle_thumb() {
        ...
        local redcircles=
        if [ -f "$radecfile" ]; then
          local redcircles=`awk '{if (NF>1) {x=$1-1+'$thumb_edge';y='$thumb_size'+'$thumb_edge'-$2;n=$4;\
