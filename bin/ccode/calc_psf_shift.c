...
int refine_mxy(int nx, int ny, long nk, float *im, long *idx, float *mx, float *my, float *flx, float *dflx, float fwhm) {
    ...
    /*printf("%d %d %ld\n",nx,ny,nk);*/
    /* fac = 0.22; = 0.5/sigma^2 = 4*log(2)/fwhm^2 */
...
int calc_psf(int nx, int ny, long nk, float *im, float *wim, long *seg, float *psf, long *idx, float *mx, float *my, float *flx, float *dflx, float raper0, float gain, int *flags) {
    /* local variables */
    int x,y,x1,y1,ymin,ymax,xmin,xmax,ki,kj,imin,imax,jmin,jmax;
    int xp,yp,sz,mx0,my0,ignore,flagged,irad;
    unsigned long k,k0,jj1;
    float dx,dy,dx1,dy1,dys,r,rmin,rmax;
    float tmp,pix,pix0,a,ai,var0,var1,psf0,psf0x,psf0y;
    float *L4dx,*L4dy,*dL4dx,*dL4dy,la,la0,flx0,flx0x,flx0y,vflx0,flx1,vflx1,norm;
    float m11,m12,m13,m23,m22,m33;
    float a11,a12,a13,det; /*a23,a22,a33*/
    float raper=3*raper0,spsf;

    L4dx = (float *) malloc(8 * sizeof(float));
    dL4dx = (float *) malloc(8 * sizeof(float));
    L4dy = (float *) malloc(8 * sizeof(float));
    dL4dy = (float *) malloc(8 * sizeof(float));
    ...
    for (k0=0;k0<nk;k0++) {
        ...
        for(ki=0;ki<8;ki++) {
            ...
            dL4dx[ki] = (dx1==0) ? 0 : ( 12.56637*dx1*sin(0.7854*dx1)*cos(3.14159*dx1) + sin(3.14159*dx1)*(3.14159*dx1*cos(0.7854*dx1)-8*sin(0.7854*dx1)) )/(9.8696*dx1*dx1*dx1);
            L4dy[ki] = (dy1==0) ? 1 : sin(3.14159*dy1)*sin(0.7854*dy1)/(2.4674*dy1*dy1);
            dL4dy[ki] = (dy1==0) ? 0 : ( 12.56637*dy1*sin(0.7854*dy1)*cos(3.14159*dy1) + sin(3.14159*dy1)*(3.14159*dy1*cos(0.7854*dy1)-8*sin(0.7854*dy1)) )/(9.8696*dy1*dy1*dy1);
        }

        /* calculate aperture and psf fluxes */
        spsf=0; flx0=0; flx0x=0; flx0y=0; vflx0=0; flx1=0; vflx1=0; norm=0; flagged=0;
        m11=m12=m13=m23=m22=m33=0;
        for (y=ymin;y<=ymax;y++) {
            dy = y-my[k]+1;
            dys = dy*dy;
            yp = y-my0+irad;
                    ...
                    if (la>0) {
                        ...
                        if (la0>0) {
                            for(kj=jmin;kj<=jmax;kj++) {
                              jj1 = xp + sz*(yp + kj);
                              for(ki=imin;ki<=imax;ki++) {
                                  psf0 += L4dx[ki+3]*L4dy[kj+3]*psf[jj1+ki];
                                  psf0x += dL4dx[ki+3]*L4dy[kj+3]*psf[jj1+ki];
                                  psf0y += L4dx[ki+3]*dL4dy[kj+3]*psf[jj1+ki];
                              }
                            }
                            psf0 *= la0;
                            psf0x *= la0;
                        ...
                        if (pix>0) pix0 = pix;
                        if (psf0>0) {
                            var1 = var0*( 1 + flx[2*k]*psf0/gain );
                            tmp = psf0/var1;
                            flx0 += tmp*la0*pix;
                            flx0x += psf0x/var1*la0*pix;
                            flx0y += psf0y/var1*la0*pix;
                            m11 += psf0*psf0/var1;
                            m12 += psf0*psf0x/var1;
                            m13 += psf0*psf0y/var1;
                            m22 += psf0x*psf0x/var1;
                            m23 += psf0x*psf0y/var1;
                            m33 += psf0y*psf0y/var1;
                            norm += tmp*psf0;
                            spsf += psf0;
                            vflx0 += tmp*tmp*la0*la0*var0*(1+pix0/gain);
                        }
                        flx1 += la*pix;
                        vflx1 += la*la*var0;
                    }
        ...
        if (norm>0) {
            det = m11*(m33*m22-m23*m23)-m12*(m33*m12-m23*m13)+m13*(m23*m12-m22*m13);
            if (det>0) {
                a11=m33*m22-m23*m23; a12=-m33*m12+m23*m13; a13=m23*m12-m22*m13;
                /*a22=m33*m11-m13*m13; a23=-m23*m11+m12*m13; a33=m22*m11-m12*m12;*/
                flx0 = (a11*flx0 + a12*flx0x + a13*flx0y)*norm/det; /*note: norm=m11*/
            }
...
int main(int argc, char *argv[])
{
    ...
    float x,y,raper,raper1,gain=1.,phot_diam=6.,dis2,dis2_min,var_min=1.,flx0_fac=0.;
          ...
          for (kj=0;kj<nbiny;kj++) {
              for (ki=0;ki<nbinx;ki++) {
                  ...
                  if (maxx[jj]>minx[jj] && maxy[jj]>miny[jj]) {
                      for (j=0;j<sz;j++) {
                         j1 = ki*sz + sz*nbinx*(j + kj*sz);
                         for (i=0;i<sz;i++) psf[i+sz*j] = psf_stack[i+j1];
                      }
                      firstpix[0] = minx[jj]+1; firstpix[1] = miny[jj]+1;
                      lastpix[0] = maxx[jj]+1; lastpix[1] = maxy[jj]+1;
                      fits_read_subset(afptr, TFLOAT, firstpix, lastpix, inc, NULL, apix, NULL, &status);
                  ...
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, flags_final);
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, flags_final);
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, flags_final);
              }
          }
