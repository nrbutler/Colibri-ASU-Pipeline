...
int main(int argc, char *argv[])
{
    ...
    if (argc < 6) { 
      printf("Usage: imreduce datafile biasfile flatfile outfile outfile_wt <siggma2> <biaslevel1> <biaslevel2> <biaslevel3> <biaslevel4> <smt>\n");
      printf("\n");
    ...
    if (check && !fits_create_file(&outfptr, argv[4], &status) && !fits_create_file(&outfptrw, argv[5], &status))
    {
      ...
      for (firstpix[1] = 1; firstpix[1] <= nrows; firstpix[1]++) {
        ...
        for(ii=0; ii< npixels; ii++) {
          ...
          rnum = rand()/rm;
          if (dpix[ii]>flat_min) {
              opix[jj] = (apix[ii]+rnum-bpix[ii]-biaslevel0)/(exptime*dpix[ii]);
              wpix[jj] = 1./sigma2;
          }
