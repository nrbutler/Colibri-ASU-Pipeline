...
int calc_psf(int nx, int ny, long nk, float *im, float *wim, long *seg, float *psf, long *idx, float *mx, float *my, float *flx, float *dflx, float raper0, float gain, float spsf, int *flags) {
    /* local variables */
    ...
    float pix,pix0,a,ai,var0,var1,psf0,gain_eff;
    float *L4dx,*L4dy,la,flx0,vflx0,flx1,vflx1,norm,flux,dflux,sum1,sum2,sum3,sum4,sum5;
    ...
    for (k0=0;k0<nk;k0++) {
        ...
        flx0=0; vflx0=0; flx1=0; vflx1=0; norm=0; flagged=0;
        for (y=ymin;y<=ymax;y++) {
            dy = y-my[k]+1;
            dys = dy*dy;
            yp = y-my0+irad;
                ...
                if (var0>0) {
                    ...
                    if (la>0) {
                        ...
                        if (psf0>0) {
                            ...
                            var1 = var0*( 1 + flux/gain_eff );
                            sum1 += pix*la/var1;
                            sum2 += psf0*la/var1;
                            sum3 += la/var1;
                            flx0 += psf0*pix*la/var1;
                            norm += psf0*psf0*la/var1;
                            vflx0 += psf0*psf0*var0*la*la*(1+pix0/gain) / (var1*var1);
                            sum4 += psf0*var0*la*la*(1+pix0/gain)/var1;
                            sum5 += var0*la*la*(1+pix0/gain)/var1;
                        }
                        flx1 += la*pix;
                        vflx1 += la*la*var0;
                    }
        ...
        if (sum3>0) norm -= sum2*sum2/sum3;
        if (norm>0) {
            flx[2*k] = flx0*spsf/norm;
            if (sum3>0) flx[2*k] -= sum1*sum2/sum3*spsf/norm;
            vflx0 += -2*sum2/sum3*sum4 + (sum2/sum3)*(sum2/sum3)*sum5;	
            dflx[2*k] = sqrt(fabs(vflx0))*spsf/norm;
        } else {
            fprintf(stderr,"Warning: negative norm!\n");
            flx[2*k] = flx[2*k+1];
            dflx[2*k] = dflx[2*k+1];
        }
        flags[k] = flagged;
      ...
      if (make_calc) {
          ...
          for (kj=0;kj<nbiny;kj++) {
              for (ki=0;ki<nbinx;ki++) {
                  ...
                  if (maxx[jj]>minx[jj] && maxy[jj]>miny[jj]) {
                      ...
                      for (j=0;j<sz;j++) {
                         ...
                         for (i=0;i<sz;i++) {
                             ...
                             if (rmin<3*raper) {
                                 rmax = r*(1+a);
                                 la = (3*raper<rmax) ? (3*raper-rmin)/(rmax-rmin) : 1;
                                 spsf += la*psf[i+sz*j];
                             }
                         }
                      }
                  ...
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, spsf, flags_final);
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, spsf, flags_final);
                  calc_psf(maxx[jj]-minx[jj]+1,maxy[jj]-miny[jj]+1, nixy[jj], apix, bpix, cpix, psf, idx_ar+cnixy[jj], mx, my, flx, dflx, raper, gain*var_min, spsf, flags_final);
              }
          }
