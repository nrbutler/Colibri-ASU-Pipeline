...
{
    fitsfile *afptr, *cfptr, *dfptr, *outfptr, *outfptrw;  /* FITS file pointers */
    ...
    long anaxes[2] = {1,1};
    double *apix, *cpix, *dpix, *opix, *wpix;
    double biaslevel0,biaslevel[2],biaslevel1=0.,biaslevel2=0.,biaslevel3=0.,biaslevel4=0., sigma2;
    double flat_min=0.5,exptime=1.,mjd=0.,rm=RAND_MAX,rnum;

    if (argc < 6) { 
      printf("Usage: imreduce datafile darkfile flatfile outfile outfile_wt <sigma2> <biaslevel1> <biaslevel2> <biaslevel3> <biaslevel4> <smt>\n");
      printf("\n");
    ...
    fits_open_file(&cfptr, argv[2], READONLY, &status);
    ...
    if (check && !fits_create_file(&outfptr, argv[4], &status) && !fits_create_file(&outfptrw, argv[5], &status))
    {
      ...
      if (apix == NULL || cpix == NULL || opix == NULL || wpix == NULL) {
        printf("Memory allocation error\n");
        return(1);
      }
      if (argc>6) sigma2=atof(argv[6]);
      if (argc>7) biaslevel1=atof(argv[7]);
      if (argc>8) biaslevel2=atof(argv[8]);
      if (argc>9) biaslevel3=atof(argv[9]);
      if (argc>10) biaslevel4=atof(argv[10]);
      if (argc>11) smt=atoi(argv[11]);
      ...
      for (firstpix[1] = 1; firstpix[1] <= nrows; firstpix[1]++) {
        ...
        for(ii=0; ii< npixels; ii++) {
          ...
          rnum = rand()/rm;
          if (dpix[ii]>flat_min) {
              opix[jj] = (apix[ii]+rnum-cpix[ii]*exptime-biaslevel0)/(exptime*dpix[ii]);
              wpix[jj] = 1./sigma2;
          }
