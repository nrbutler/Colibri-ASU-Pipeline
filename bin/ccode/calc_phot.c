...
int calc_aper(long nx, long ny, long nk, float *im, float *wim, long *seg, float *mx, float *my, float raper, float gain, float *flx, float *dflx, int *flags, int ap) {
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 5) {
      printf("Usage: calc_phot datafile weightfile segfile catfile [crpix1] [crpix2]\n");
      printf("\n");
      return(0);
    }
    ...
    if (check) {
      ...
      fits_update_key(dfptr, TFLOAT, "FWHM", &fwhm, NULL, &status);
      fits_update_key(dfptr, TFLOAT, "EXPTIME", &exptime, NULL, &status);
      fits_update_key(dfptr, TSTRING, "T0", dobs, NULL, &status);
      fits_update_key(dfptr, TSTRING, "T1", dobe, NULL, &status);
      if (status!=0) status=0;
      ...
      x0=0; y0=0;
      if (argc>5) x0=(int)(atof(argv[5])-x);
      if (argc>6) y0=(int)(atof(argv[6])-y);
      ...
      calc_aper(anaxes[0], anaxes[1], nrows, apix, bpix, cpix, mx, my, raper1, gain*var_min, flx, dflx, flags_final, 0);

      fprintf(stderr," Extracting flux for %s in %.1f pixel radius aperture...\n",argv[1],raper2);
      calc_aper(anaxes[0], anaxes[1], nrows, apix, bpix, cpix, mx, my, raper2, gain*var_min, flx, dflx, flags_final, 1);
