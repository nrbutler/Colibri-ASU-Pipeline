...
{
    fitsfile *afptr, *bfptr, *cfptr, *dfptr, *psffile, *outfptr, *outfptr_wt;  /* FITS file pointers */
    ...
    if (check) {
      ...
      if (argc>8) filter = atoi(argv[8]);

      /* this is a smoothing radius, limiting the psf fitting */
      if (argc>9) rad = atoi(argv[9]);
      if (rad<0) rad = irad;
            ...
            for (i0=irad_max0+1+j0;i0<a-irad_max0+j0;i0++) {
              ...
              if (det>0) {
                ...
                if (dis2<1) {
                    ...
                    cim0 += delta_x*dx[i0] + delta_y*dy[i0];
                }
              }
              cim0 = fabs(cim0);
